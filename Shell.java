...
public class Shell {
    ...
    public static void main(String[] args) {
        ...
        Shell anObject = new Shell();
        anObject.shellIt(studentID);
        // Print sorted array
        int j = 1;
    ...
    int shellIt (int array[]) {

        int len = array.length;
        // Reduce shift size
        for (int shiftSize = len/2; shiftSize > 0; shiftSize /= 2) {
            // Insert elements until array is sorted
            for (int i = shiftSize; i < len; i += 1) {
                // save element value && create empty element
                int tElement = array[i];

                // shift until tElement's place is located
                int j;
                for (j = i; j >= shiftSize && array[j - shiftSize] > tElement; j -= shiftSize)
                    array[j] = array[j - shiftSize];

                // put tElement at proper index
                array[j] = tElement;
            }
        }
        return 0;
